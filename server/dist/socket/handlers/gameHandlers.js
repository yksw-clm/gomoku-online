...
exports.setupGameHandlers = exports.cleanupPlayerGames = void 0;
...
const waitingGames = [];
// ユーザーごとに作成したゲームを記録
const userCreatedGames = new Map();
// タイマーを保存
const gameTimers = new Map();
// タイマーの時間（ミリ秒）
const MOVE_TIME_LIMIT = 30000; // 30秒
const cleanupPlayerGames = (io, playerName) => {
...
exports.cleanupPlayerGames = cleanupPlayerGames;
// タイマーのセットアップ関数
const setupMoveTimer = (io, gameId) => {
    // 既存のタイマーがあればクリア
    if (gameTimers.has(gameId)) {
        clearTimeout(gameTimers.get(gameId));
    }
    ...
    const timer = setTimeout(() => {
        ...
        const otherPlayerIndex = game.players.findIndex((p) => p.name !== currentPlayer.name);
...
const setupGameHandlers = (io) => {
    io.on("connection", (socket) => {
        ...
        socket.on("createGame", (data) => {
            ...
            const player = new Player_1.Player(playerName, "black"); // 先手は黒
            const game = new Game_1.Game(gameId, player);
        ...
        socket.on("joinGame", (data) => {
            ...
            // 2人目のプレイヤーは白（後手）
            const player = new Player_1.Player(playerName, "white");
            ...
            io.to(gameId).emit("timerUpdate", {
                timeRemaining: MOVE_TIME_LIMIT / 1000,
        ...
        socket.on("forfeitGame", (data) => {
            ...
            if (game) {
                ...
                io.to(gameId).emit("playerForfeit", {
                    message: `${playerId}が降参しました。${winner ? winner.name + "の勝ちです。" : ""}`,
                    winner: winner?.name,
                    gameState: game.getGameState(),
                });
                // ゲームを削除
...
const broadcastWaitingGames = (io) => {
    const games = waitingGames.map((gameId) => {
        const game = activeGames.get(gameId);
        return {
            id: gameId,
